...
def get_file_before_afters_for_selection(
    generated_test_executions: GeneratedTestExecutionResults,
) -> list[list[ChangedFile]]:
    return generated_test_executions.file_before_afters

def get_patches_for_selection(
    generated_test_executions: GeneratedTestExecutionResults,
    max_candidates: int,
    regression_test_executions: RegressionTestExecutionResults | None = None,
) -> list[PatchInfo]:
    patch_votes_pairs = get_patch_vote_pairs(
        generated_test_executions,
    )
    if regression_test_executions is not None:
        for index, (patch, votes) in enumerate(patch_votes_pairs):
            # assuming the patches indexes are the same
            passing_tests = regression_test_executions.passing_tests_per_edit[index]
            if len(passing_tests) > 0:
                print(f"Adding {len(passing_tests)} passing tests to patch {index}")
                patch_votes_pairs[index] = (patch, votes + len(passing_tests))
