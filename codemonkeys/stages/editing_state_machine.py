...
from codemonkeys.trajectory_data.structs import EditingStateData, Message, RelevantChunks
...
class EditingStateMachine(
    ...
    def __init__(self, work_item: EditingWorkItem, config: EditingStateMachineConfig):
        ...
        self.relevant_chunks = work_item.relevant_chunks
    ...
    def write_initial_edit(self):
        system_prompt_with_files = system_prompt_with_file_data(
            self.problem, self.context.files, self.relevant_chunks
        )
    ...
    def validate_initial_edit(self):
        ...
        match parsed:
            case str():
                print("STATE MACHINE EMITTED EDIT")
...
def get_work_items(
    ...
    for problem in problems:
        ...
        if trajectory_store.relevant_chunks_exists():
            relevant_chunks = trajectory_store.load_relevant_chunks()
        else:
