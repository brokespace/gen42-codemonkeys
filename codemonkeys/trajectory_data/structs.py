...
class RelevantLocations(BaseModel):
    message: str
    locations: list[str]
...
class ProblemContextChunks(BaseModel):
    chunks: list[RelevantChunks]
...
class RegressionTestExecutionResults(BaseModel):
    passing_tests_on_empty_diff: list[str]
    passing_tests_per_edit: list[list[str]]
...
class EditingStateData(StateData):
    ...
    file_before_afters: list[list[ChangedFile]] = Field(default_factory=list)
