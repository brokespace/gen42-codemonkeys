...
class TrajectoryDataKeys(str, Enum): # TODO move regression test execution to before 
    RELEVANCE_DECISIONS = "01_relevance_decisions"
    LOCALIZATION_DECISIONS = "02_localization_decisions"
    RELEVANT_CHUNKS = "02_relevant_chunks"
    ...
    REGRESSION_TEST_EXECUTION = "11_regression_test_execution.json"
...
class TrajectoryStore:
    ...
    def relevant_chunks_exists(self) -> bool:
        return self._relevant_chunks_data_path().exists()
    
    def _relevant_chunks_data_path(self, codebase_file_path: str = "") -> Path:
        if codebase_file_path == "":
            return self._base_dir() / TrajectoryDataKeys.RELEVANT_CHUNKS
        else:
            return (
                self._base_dir()
                / TrajectoryDataKeys.RELEVANT_CHUNKS
                / f"{codebase_file_path}.json"
            )
        
    def save_per_file_relevant_chunks(
        self, codebase_file_path: str, relevant_chunks: RelevantChunks
    ):
        _save_to_file(
            relevant_chunks,
            self._relevant_chunks_data_path(codebase_file_path),
        )

    def load_relevant_chunks(self) -> list[RelevantChunks]:
        relevant_chunks = []

        for codebase_file_path in self.problem.all_file_paths():
            data_path = self._relevant_chunks_data_path(codebase_file_path)
            if data_path.exists():
                relevant_chunks.append(_load_from_file(
                    RelevantChunks, data_path
                ))
    ...
    def _localization_decision_data_path(self, codebase_file_path: str) -> Path:
        return (
            self._base_dir()
            / TrajectoryDataKeys.LOCALIZATION_DECISIONS
            / f"{codebase_file_path}.json"
        )
    
    def localization_decision_exists(self, codebase_file_path: str) -> bool:
        return self._localization_decision_data_path(codebase_file_path).exists()
    
    def save_localization_decision(self, codebase_file_path: str, localization_decision: RelevantLocations):
        _save_to_file(localization_decision, self._localization_decision_data_path(codebase_file_path))
        
    def load_localization_decision(self, codebase_file_path: str) -> RelevantLocations:
        return _load_from_file(RelevantLocations, self._localization_decision_data_path(codebase_file_path))
    
    
    def load_context(self, max_tokens: int = 0) -> list[CodebaseFile]:
        if self.ranking_exists():
            ...
            for file_data in context_data.ranked_files:
                if max_tokens > 0 and file_data.tokens + context_tokens > max_tokens:
                    break
            ...
            # If ranking doesn't exist, use only relevant files from the problem
            context = []
            relevance_decisions = self.load_relevance_decisions()
            
            for file_path, decision in relevance_decisions.items():
                if decision.status == "relevant":
                    context.append(self.problem.get_file(file_path))
            
            return context
    
    def load_relevant_files(self) -> list[CodebaseFile]:
        relevance_decisions = self.load_relevance_decisions()
        return [self.problem.get_file(file_path) for file_path, decision in relevance_decisions.items() if decision.status == "relevant"]
    ...
    def save_regression_test_execution(self, results: RegressionTestExecutionResults):
        _save_to_file(
            results,
            self._regression_test_execution_path(),
        )

    def regression_test_execution_exists(self) -> bool:
        return self._regression_test_execution_path().exists()

    def load_regression_test_execution(self) -> RegressionTestExecutionResults:
        print("loading regression test execution", self._regression_test_execution_path())
        return _load_from_file(
            RegressionTestExecutionResults,
            self._regression_test_execution_path(),
        )

    def _regression_test_execution_path(self) -> Path:
        return self._base_dir() / TrajectoryDataKeys.REGRESSION_TEST_EXECUTION
